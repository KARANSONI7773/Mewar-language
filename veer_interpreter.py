...
class VeerInterpreter:
    ...
    def run(self, code):
        ...
        while self.program_counter < len(lines):
            ...
            # --- PARSER UPGRADE ---
            # Don't just split by space. Find the command and the rest of the line.
            command_match = re.match(r"(\w+)", line)
            if not command_match:
                continue
            
            command = command_match.group(1)
            # The rest of the line is the arguments string
            args_str = line[len(command):].strip()

            try:
                if command in self.functions: self.execute_function_call(command)
                # Pass the whole argument string to execute_say
    ...
    def execute_say(self, args_str):
        """NEW: A more robust say command that properly handles quoted strings."""
        if not args_str:
            print() # Handles `say` with no arguments, printing a blank line
            return
        ...
        output = []
        for part in parts:
            value = self.get_value(part.strip())
            output.append(str(value))
        
        print(" ".join(output))

    # The rest of the methods are unchanged from v1.0...
    ...
    def execute_set(self, line):
        ...
        if '[' in var_name_part[1]:
            match = re.match(r"(\w+)\[(.*)\]", var_name_part[1]); list_name, index_expr = match.groups()
            index = self.get_value(index_expr); value = self.get_value(parts[1])
            self.variables[list_name][index - 1] = value; return
        var_name = var_name_part[1]; value_str = parts[1].strip()
        if value_str.startswith('[') and value_str.endswith(']'):
            list_contents = value_str[1:-1].split(','); self.variables[var_name] = [self.get_value(item) for item in list_contents]
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Veer Interpreter v1.1"); print("Usage: python veer.py <filename.mewar>")
    else:
